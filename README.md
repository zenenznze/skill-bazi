...
临时试算、即时占、一次性排盘和未归属档案的中间产物统一放入 `temp/`，并由 `.gitignore` 忽略。
若用户明确要求在本地记录档案，应保存到本地私有 `work/people/<name>/` 或对应案例目录；公开发布时不得带入个人档案。
...
- `bazi-renji` 新增“群体互动断法：比劫、官杀、印星三件套”，用于分析合作僵局、同级竞争、责任分散和让步问题。
- `bazi-shiye` 新增“会议室领导力断法”，将七杀、正官、食伤、财星、印星、比劫翻译成组织场里的定方向、立规则、破僵局、抓结果和守分寸。
- `bazi-shiye` SOP 增加领导力、管理岗、会议文化、管理类型的触发和细分规则。
- 新增临时产物流程：未归属档案的即时分析统一进入 `temp/`，并由 `.gitignore` 忽略；用户要求记录档案时再整理进本地私有 `work/people/<name>/`。
- 同步主入口、命局、格局、人际、事业等模块的最新通用规则；公开版仍不包含个人案例档案。
